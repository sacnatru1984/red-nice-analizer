        ...
        {sel && (() => {
          ...
          <div>
            <div style={{ background: 'linear-gradient(135deg,#1E3A8A 0%,#2563EB 100%)', borderRadius: 10, padding: '18px 20px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 16, flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
              ...
              <button onClick={() => exportAffiliateReport(sel, sig, pct, getChecks(), contactosParaExport(), pasos).catch(e=>{console.error(e); alert('No se pudo generar el plan para descargar. Intenta de nuevo.')})} title="Descargar plan como imagen" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 14px', borderRadius: 8, background: 'rgba(255,255,255,.16)', border: '1px solid rgba(255,255,255,.3)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0, width: isMobile ? '100%' : 'auto', order: isMobile ? 1 : 0 }}>
